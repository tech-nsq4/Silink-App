...
class CompanyCatalogScreen extends StatelessWidget {
        ...
        return Scaffold(
          body: SafeArea(
            child: Column(
              children: [
                ScreenHeaderBar(
                  title: LocaleKeys.company_catalog.tr(),
                ),
                Expanded(
                  child: ListView.separated(
                    padding: EdgeInsets.all(19.w),
                    itemCount: state.company.catalog.length,
                    separatorBuilder: (_, __) =>
                        SizedBox(height: 8.h),
                    itemBuilder: (context, index) {
                      final item = state.company.catalog[index];
                      return ListTile(
                        title: AppText(item),
                        trailing: IconButton(
